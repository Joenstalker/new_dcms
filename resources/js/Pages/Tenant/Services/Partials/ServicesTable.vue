                ...
                <tbody>
                    <tr v-for="service in services" :key="service.id" class="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="text-right space-x-2">
                            <!-- Regular Actions -->
                            <button v-if="canManage" @click="handleEdit(service)" class="btn btn-sm btn-ghost hover:bg-indigo-50 text-indigo-600">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                </svg>
                            </button>
                            <button v-if="canManage" @click="handleDelete(service.id)" class="btn btn-sm btn-ghost hover:bg-red-50 text-red-500">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <tr v-if="services.length === 0">
                        <td colspan="4" class="py-20 text-center">
                            <div class="flex flex-col items-center gap-2 opacity-40">
                                ...
                                <p class="text-xl font-bold">No services found.</p>
                            </div>
                        </td>
                    </tr>
                </tbody>
